...
export function RunsPage() {
  const { runs, currentProject, environment, smokeIntent, clearSmokeIntent, startRun, viewMode } = useApp();
  const [selected, setSelected] = useState<TestRun | null>(null);
  const filteredRuns = useMemo(() => runs.filter((run) => run.projectId === currentProject.id && run.environment === environment), [currentProject.id, environment, runs]);
  useEffect(() => {if (selected) setSelected(runs.find((run) => run.id === selected.id) ?? null);}, [runs, selected]);
  useEffect(() => {if (selected && (selected.projectId !== currentProject.id || selected.environment !== environment)) setSelected(null);}, [currentProject.id, environment, selected]);
  ...
  return <div className="mx-auto max-w-7xl px-4 py-6"><div className="mb-4"><h1 className="text-xl font-semibold text-ink">Runs</h1><p className="mt-0.5 text-sm text-ink-2">{viewMode === "quick" ? `Confirm a ${environment} Smoke run, then investigate the result in one place.` : `Trigger and inspect ${environment} runs for ${currentProject.name}.`}</p></div><div className="mb-5"><QuickRun intent={smokeIntent} onRun={begin} /></div>{filteredRuns.length === 0 ? <div className="rounded-xl border border-line bg-surface"><EmptyState icon={InboxIcon} title={`No ${environment} runs yet`} description="Use Quick Run above or select another environment to inspect its history." /></div> : <div className="overflow-hidden rounded-xl border border-line bg-surface"><div className="overflow-x-auto"><table className="w-full text-sm"><thead className="sticky top-0 z-10 bg-surface-2 text-ink-3"><tr className="text-left text-2xs uppercase tracking-wide"><Th className="pl-4">Run</Th><Th>Pack</Th><Th className="hidden md:table-cell">Environment</Th><Th className="hidden lg:table-cell">Triggered by</Th><Th className="hidden xl:table-cell">Build</Th><Th className="min-w-[180px]">Results</Th><Th className="hidden sm:table-cell">Duration</Th><Th className="hidden lg:table-cell pr-4">Started</Th></tr></thead><tbody className="divide-y divide-line">{filteredRuns.map((run) => <RunRow key={run.id} run={run} highlight={selected?.id === run.id} onClick={() => setSelected(run)} />)}</tbody></table></div></div>}<RunDetailDrawer run={selected} onClose={() => setSelected(null)} /></div>;
}
